...
!!! dirty reshape arrays indexing ijk -> kji !!!
    ...
    if (check_output) then
       ...
       !if ((iter_solve .EQ. 199) .OR. (iter_solve .EQ. 200)) then 
          ...
          call write_netcdf(u,vname='ruin',netcdf_file_name='so.nc',rank=myrank,iter=iter_solve)
          call write_netcdf(v,vname='rvin',netcdf_file_name='so.nc',rank=myrank,iter=iter_solve)
          call write_netcdf(w,vname='rwin',netcdf_file_name='so.nc',rank=myrank,iter=iter_solve)
          else
    ...
    if (check_output) then
       ...
       !if ((iter_solve .EQ. 199) .OR. (iter_solve .EQ. 200)) then
          if ((present(rufrca)).and.(present(rvfrca))) then
          call write_netcdf(u,vname='ruout',netcdf_file_name='so.nc',rank=myrank,iter=iter_solve)
          call write_netcdf(v,vname='rvout',netcdf_file_name='so.nc',rank=myrank,iter=iter_solve)
          call write_netcdf(w,vname='rwout',netcdf_file_name='so.nc',rank=myrank,iter=iter_solve)
          else
