...
!!!
  ...
  !--------------------------------------------------------------
  subroutine nhmg_solve(nx,ny,nz,ua,va,wa,Hza,Hzha,rua,rva,rwa,dt,rufrca,rvfrca)
    ...
    real(kind=rp), dimension(0:nx+1,1:ny+1,1:nz  ), target, intent(inout) :: va
    real(kind=rp), dimension(0:nx+1,0:ny+1,1:nz+1), target, intent(inout) :: wa
    real(kind=rp), dimension(0:nx+1,0:ny+1,1:nz+1), target, optional, intent(in) :: Hza
    real(kind=rp), dimension(0:nx+1,0:ny+1,1:nz+1), target, optional, intent(in) :: Hzha
...
!!! dirty reshape arrays indexing ijk -> kji !!!
    ...
    v => vb
    w => wb
    if ((present(Hza)).and.(present(Hzha))) then
       allocate(Hzb(1:nz  ,0:ny+1,0:nx+1))
       allocate(Hzhb(1:nz  ,0:ny+1,0:nx+1))
       do i = 0,nx+1
          do j = 0,ny+1
             do k = 1,nz
                Hzb(k,j,i) = Hza(i,j,k)
       ...
       do i = 0,nx+1
          do j = 0,ny+1
             do k = 1,nz
                Hzhb(k,j,i) = Hzha(i,j,k)
             enddo
          enddo
       enddo
       Hz => Hzb
       Hzh => Hzhb
       call fill_halo(1,Hzh)
    endif
!!!
...
!!$       endif
       ...
       call bc2bt_coupling(ru,rv,rw,Hz,Hzh,dt,rufrc,rvfrc)
